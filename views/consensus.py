# -*- coding: utf-8 -*-
"""🎯 증권사 목표가 컨센서스 패널 — 종목 상세의 한 탭.

[통합] 예전에는 독립 메뉴로 자체 종목 선택박스를 가졌다.
지금은 views/stock_detail.py 가 고른 종목을 받아 분석만 한다. 표출 로직은 그대로다.
"""
from core import *


def panel(ctx, name, code, is_us):
    """선택된 종목의 증권사 목표가 컨센서스."""
    api_key_input = ctx.get('api_key_input', "")

    if is_us:
        st.info("증권사 컨센서스는 국내 종목만 지원합니다. (네이버 금융 리서치 기반)")
        return

    q_name, q_code = name, code
    st.caption("최근 6개월간 발간된 증권사 리포트의 목표가 추이와 투자의견 분포입니다.")
