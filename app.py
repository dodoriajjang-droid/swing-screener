...
# [추가] 구글 시트(Google Sheets) 연동용 — 서비스 계정 방식 (pip install gspread google-auth)
try:
    import gspread
    from google.oauth2.service_account import Credentials as _GCreds
    try:
        from gspread.utils import ValueInputOption as _VIO
        _USER_ENTERED = _VIO.user_entered          # gspread 6.x str-enum
    except Exception:
        _USER_ENTERED = "USER_ENTERED"
    HAS_GSPREAD = True
    GSPREAD_IMPORT_ERR = ""
except Exception as e:
    gspread = None
    ...
    _USER_ENTERED = "USER_ENTERED"
    HAS_GSPREAD = False
    GSPREAD_IMPORT_ERR = f"{type(e).__name__}: {e}"
...
# ==========================================
# 0-B. 구글 시트(Google Sheets) 연동 — 서비스 계정(비공개·양방향)
#   - 관심종목 탭에서 종목을 읽고(⬇️), 현재 관심종목을 저장하고(⬆️),
#     시세·신호 스냅샷을 모니터링 탭에 기록(📤)한다.
#   - 인증 정보는 st.secrets['gcp_service_account'] (서비스 계정 JSON)에서 읽는다.
# ==========================================
GSHEET_WL_WS = "관심종목"      # 읽기/쓰기 대상 워크시트(탭)
GSHEET_MON_WS = "모니터링"     # 모니터링 스냅샷 기록 탭
_GSHEET_SCOPES = ["https://www.googleapis.com/auth/spreadsheets",
                  "https://www.googleapis.com/auth/drive"]

def _gsheet_secret(key, default=None):
    try:
...
def _gspread_client_cached(sa_json_str):
    """서비스 계정 JSON(문자열)으로 gspread 클라이언트 생성. 캐시 키=JSON 문자열.
    실패 시 예외를 던져 캐시되지 않게 함(→ secrets 수정 후 자동 재시도)."""
    info = json.loads(sa_json_str)
    creds = _GCreds.from_service_account_info(info, scopes=_GSHEET_SCOPES)
    return gspread.authorize(creds)

def _get_gspread_client():
    """반환 (client, None) 성공 / (None, '사유') 실패. authorize 단계는 네트워크를 타지 않음."""
    if not HAS_GSPREAD:
        return None, f"gspread 미설치 — 터미널에서 `pip install gspread google-auth` 후 앱을 재시작하세요. ({GSPREAD_IMPORT_ERR})"
    sa = _gsheet_secret("gcp_service_account")
    if not sa:
        return None, "secrets에 [gcp_service_account] 가 없습니다. 아래 '최초 1회 설정 가이드'를 따라 추가하세요."
    try:
        sa_json = json.dumps(dict(sa), sort_keys=True)   # 해시 가능한 캐시 키
        return _gspread_client_cached(sa_json), None
    except Exception as e:
        return None, f"서비스 계정 인증 실패: {type(e).__name__}: {e}"

def _service_account_email():
    sa = _gsheet_secret("gcp_service_account") or {}
    try:
        return dict(sa).get("client_email", "")
    except Exception:
        return ""

def _open_sheet(sheet_url):
    """URL 또는 키로 스프레드시트 open. 반환 (spreadsheet, None)/(None, '사유')."""
    client, err = _get_gspread_client()
    if err:
        return None, err
    url = (sheet_url or _gsheet_secret("gsheet_url") or "").strip()
    if not url:
        return None, "시트 URL이 비어 있습니다. 위 입력칸에 스프레드시트 주소를 붙여넣으세요."
    try:
        return (client.open_by_url(url) if url.startswith("http") else client.open_by_key(url)), None
    except Exception as e:
        return None, f"시트 열기 실패 — 서비스 계정에게 '편집자' 공유가 됐는지·URL이 맞는지 확인하세요. ({type(e).__name__})"

def gsheet_read_watchlist(sheet_url, worksheet=GSHEET_WL_WS):
    """'관심종목' 탭을 읽어 [{'종목명','티커','메모'}] 반환. (rows, None)/(None, '사유').
    첫 행을 헤더로 사용하며 헤더 표기 흔들림(종목명/name, 티커/코드/ticker 등)을 허용."""
    sh, err = _open_sheet(sheet_url)
    if err:
        return None, err
    try:
        try:
            ...
            return None, f"'{worksheet}' 탭이 없습니다. 첫 행에 [종목명 · 티커 · 메모] 헤더를 둔 탭을 만들어 주세요."
        ...
        for r in records:
            ...
            def pick(*keys):
                for k in keys:
                    ...
                    if k.lower() in low and str(low[k.lower()]).strip(): return str(low[k.lower()]).strip()
                return ""
            name = pick("종목명", "name")
            code = pick("티커", "코드", "ticker", "code", "symbol")
            memo = pick("메모", "코멘트", "memo", "note", "comment")
            if code.isdigit():
                code = code.zfill(6)          # 한국 숫자코드 앞자리 0 보존(005930 등)
            if not code or code in seen:
                continue
            seen.add(code)
            rows.append({"종목명": name or code, "티커": code, "메모": memo})
        return rows, None
    except Exception as e:
        return None, f"읽기 실패: {type(e).__name__}: {e}"

def _gsheet_cell(v):
    """gspread에 안전하게 보낼 셀 값으로 정규화(넘파이/NaN/None 처리)."""
    if v is None:
        return ""
...
def _gsheet_overwrite(ws, values):
    """워크시트를 비우고 2차원 리스트를 A1부터 기록(USER_ENTERED)."""
    ws.clear()
    ws.append_rows([[_gsheet_cell(c) for c in row] for row in values],
                   value_input_option=_USER_ENTERED)

def gsheet_write_watchlist(sheet_url, items, worksheet=GSHEET_WL_WS):
    """현재 관심종목([{'종목명','티커',('메모')}])을 '관심종목' 탭에 덮어쓰기. (n, None)/(None, '사유')."""
    sh, err = _open_sheet(sheet_url)
    if err:
        return None, err
    try:
        try:
            ...
            ws = sh.add_worksheet(title=worksheet, rows=200, cols=6)
        values = [["종목명", "티커", "메모"]]
        for it in items:
            code = str(it.get("티커", ""))
            code_cell = ("'" + code) if code.isdigit() else code   # 앞자리 0 보존(텍스트 강제)
            values.append([str(it.get("종목명", "")), code_cell, str(it.get("메모", ""))])
        _gsheet_overwrite(ws, values)
        return len(items), None
    except Exception as e:
        return None, f"쓰기 실패: {type(e).__name__}: {e}"

def build_watchlist_monitoring(items):
    """[{'종목명','티커','메모'}] → 모니터링 DataFrame.
    컬럼: 종목명·티커·시장·신호·현재가·등락률(%)·RSI·손절가·목표가·메모·갱신시각(KST).
    신호/등락률은 기존 analyze_technical_pattern 결과로 산출(홈 위젯과 동일 규칙)."""
    now_kst = (datetime.utcnow() + timedelta(hours=9)).strftime("%Y-%m-%d %H:%M")
    cols = ["종목명", "티커", "시장", "신호", "현재가", "등락률(%)", "RSI", "손절가", "목표가", "메모", "갱신시각"]
    out = []
    for it in items:
        name, code = str(it.get("종목명", "")), str(it.get("티커", ""))
        is_us = not code.isdigit()
        rec = {"종목명": name, "티커": code, "시장": ("US" if is_us else "KR"),
               "신호": "⚪ 조회실패", "현재가": None, "등락률(%)": None, "RSI": None,
               "손절가": None, "목표가": None, "메모": it.get("메모", ""), "갱신시각": now_kst}
        try:
            res = analyze_technical_pattern(name, code)
        except Exception:
            res = None
        if res:
            cur, sl, tg = float(res["현재가"]), float(res["손절가"]), float(res["목표가1"])
            if cur <= sl:            sig = "🔴 손절위험"
            elif cur >= tg * 0.98:   sig = "🟢 익절구간"
            else:                    sig = "🟡 홀딩"
            # 등락률: 반환된 '차트 데이터'(최근 20봉)에서 전일 종가 대비 산출
            chg = None
            try:
                closes = res["차트 데이터"]["Close"]
            ...
            rsi = None
            try:
                rsi = round(float(res.get("RSI")), 1)
                if rsi >= 70:   sig += " · 🔥RSI과열"
                elif rsi <= 30: sig += " · 🧊RSI과매도"
            except Exception:
                rsi = None
            rec.update({
                "신호": sig,
                "현재가": (round(cur, 2) if is_us else int(round(cur))),
                "등락률(%)": (round(chg, 2) if chg is not None else None),
                "RSI": rsi,
                "손절가": (round(sl, 2) if is_us else int(round(sl))),
                "목표가": (round(tg, 2) if is_us else int(round(tg))),
            })
        out.append(rec)
    return pd.DataFrame(out, columns=cols)

def gsheet_write_monitoring(sheet_url, df, worksheet=GSHEET_MON_WS):
    """모니터링 DataFrame을 '모니터링' 탭에 덮어쓰기. (n, None)/(None, '사유')."""
    sh, err = _open_sheet(sheet_url)
    if err:
        return None, err
    try:
        try:
            ...
            ws = sh.add_worksheet(title=worksheet, rows=200, cols=max(12, len(df.columns) + 1))
        ...
        for _, r in df.iterrows():
            ...
            for c in header:
                v = r[c]
                row.append(("'" + str(v)) if (c == "티커" and str(v).isdigit()) else v)
            values.append(row)
        _gsheet_overwrite(ws, values)
        return len(df), None
    except Exception as e:
        return None, f"기록 실패: {type(e).__name__}: {e}"

def _style_monitoring(df):
    """모니터링 표: 등락률 빨강(+)/파랑(-) 한국식 색상."""
    def _chg(v):
        try:
            x = float(v)
        except Exception:
            return ""
        if x > 0: return "color:#ef4444;font-weight:700;"   # 상승=빨강
        if x < 0: return "color:#3b82f6;font-weight:700;"   # 하락=파랑
        return ""
    try:
        return df.style.map(_chg, subset=["등락률(%)"])
    except Exception:
        try:
            return df.style.applymap(_chg, subset=["등락률(%)"])
        except Exception:
            return df

_GSHEET_SETUP_GUIDE = """\
**한 번만 설정하면 됩니다 (약 5분).** 서비스 계정은 비공개 시트에 안전하게 읽기/쓰기를 합니다.

**1) 구글 클라우드 콘솔** → 프로젝트 생성(또는 기존 선택) → **API 및 서비스 → 라이브러리**에서
`Google Sheets API` 와 `Google Drive API` 를 각각 **사용 설정**.

**2) 사용자 인증 정보 → 사용자 인증 정보 만들기 → 서비스 계정** 생성 →
만든 계정의 **키 → 키 추가 → 새 키 → JSON** 다운로드.

**3) `.streamlit/secrets.toml`** 에 아래 형식으로 붙여넣기 (다운로드한 JSON 값 그대로, 줄바꿈 주의):
```toml
[gcp_service_account]
...
client_email = "your-bot@your-project-id.iam.gserviceaccount.com"
client_id = "..."
token_uri = "https://oauth2.googleapis.com/token"

# (선택) 기본 시트 주소를 넣어두면 입력칸 없이 바로 연결됩니다
gsheet_url = "https://docs.google.com/spreadsheets/d/스프레드시트_ID/edit"
```

**4) 구글 시트 생성** → 탭(시트) 이름을 **`관심종목`** 으로 만들고 첫 행에 `종목명 · 티커 · 메모` 헤더 입력.
(`모니터링` 탭은 스냅샷 기록 시 자동 생성됩니다.)

**5) 그 시트의 [공유]** → 위 `client_email` 주소를 **편집자**로 추가. ← 이 단계를 빼먹으면 권한 오류가 납니다.
이 화면에 서비스 계정 이메일이 표시됩니다 → **`__SA_EMAIL__`**

**6) 한국 종목 코드**는 앞자리 0이 사라지지 않도록 텍스트로 저장됩니다(자동 처리). 미국 종목은 `AAPL` 처럼 티커를 그대로 적습니다.
"""
...
if 'wl_memos' not in st.session_state: st.session_state.wl_memos = {}   # {티커: 메모} — 구글시트 연동용
...
if selected_menu == "🎛️ 홈: 종합 대시보드":
    ...
    # ─────────── 📊 구글 시트 연동 (서비스 계정 · 비공개 · 양방향) ───────────
    with st.container(border=True):
        st.markdown("##### 📊 구글 시트 연동 (서비스 계정 · 비공개 · 양방향)")
        _default_url = _gsheet_secret("gsheet_url", "") or ""
        gsheet_url = st.text_input(
            "스프레드시트 URL",
            value=st.session_state.get("gsheet_url_input", _default_url),
            placeholder="https://docs.google.com/spreadsheets/d/.../edit",
            help="서비스 계정 이메일을 이 시트에 '편집자'로 공유해야 읽기/쓰기가 됩니다.",
        )
        st.session_state.gsheet_url_input = gsheet_url

        _cli, _cli_err = _get_gspread_client()
        _sa_email = _service_account_email()
        if _cli_err:
            st.warning(f"🔌 연결 대기 — {_cli_err}")
        elif _sa_email:
            st.success(f"🔐 서비스 계정 인증됨 · 이 시트에 **편집자**로 공유하세요 → `{_sa_email}`")

        cA, cB, cC = st.columns(3)
        if cA.button("⬇️ 시트에서 불러오기", use_container_width=True,
                     help=f"'{GSHEET_WL_WS}' 탭의 종목을 관심종목으로 가져옵니다."):
            with st.spinner("시트에서 관심종목을 읽는 중..."):
                _rows, _err = gsheet_read_watchlist(gsheet_url)
            if _err:
                st.error(_err)
            else:
                st.session_state.watchlist = [{"종목명": r["종목명"], "티커": r["티커"]} for r in _rows]
                st.session_state.wl_memos = {r["티커"]: r["메모"] for r in _rows}
                save_watchlist(st.session_state.watchlist)
                st.success(f"✅ {len(_rows)}개 종목을 시트에서 불러왔습니다.")
                st.rerun()

        if cB.button("⬆️ 시트에 저장", use_container_width=True,
                     help=f"현재 관심종목을 '{GSHEET_WL_WS}' 탭에 덮어씁니다."):
            _memos = st.session_state.get("wl_memos", {})
            _items = [{"종목명": it["종목명"], "티커": it["티커"], "메모": _memos.get(it["티커"], "")}
                      for it in st.session_state.watchlist]
            with st.spinner("시트에 관심종목을 저장하는 중..."):
                _n, _err = gsheet_write_watchlist(gsheet_url, _items)
            st.error(_err) if _err else st.success(f"✅ {_n}개 종목을 '{GSHEET_WL_WS}' 탭에 저장했습니다.")

        if cC.button("📤 모니터링 스냅샷 기록", type="primary", use_container_width=True,
                     help=f"현재 시세·신호·RSI를 '{GSHEET_MON_WS}' 탭에 기록합니다."):
            if not st.session_state.watchlist:
                st.warning("관심종목이 비어 있습니다. 먼저 불러오거나 추가하세요.")
            else:
                _memos = st.session_state.get("wl_memos", {})
                _items = [{"종목명": it["종목명"], "티커": it["티커"], "메모": _memos.get(it["티커"], "")}
                          for it in st.session_state.watchlist]
                with st.spinner("시세·신호 계산 후 시트에 기록 중..."):
                    _mon = build_watchlist_monitoring(_items)
                    _n, _err = gsheet_write_monitoring(gsheet_url, _mon)
                if _err:
                    st.error(_err)
                else:
                    st.success(f"✅ {_n}개 종목 모니터링을 '{GSHEET_MON_WS}' 탭에 기록했습니다. (KST 기준 갱신시각 포함)")
                    st.dataframe(_style_monitoring(_mon), use_container_width=True, hide_index=True)

        with st.expander("⚙️ 최초 1회 설정 가이드 (서비스 계정)"):
            st.markdown(_GSHEET_SETUP_GUIDE.replace("__SA_EMAIL__", _sa_email or "(secrets 설정 후 이 자리에 표시됩니다)"))

    st.divider()

    # ─────────── 관심종목 상세 카드 ───────────
    if not st.session_state.watchlist:
        st.info("추가된 종목이 없습니다. 위에서 **시트 불러오기**를 하거나, 스캐너·분석기에서 관심종목을 추가해보세요.")
    else:
