...
# ==========================================
# 🎨 글로벌 테마: 토스/카뱅 스타일 (Soft Fintech)
#   - 라이트 배경 + 연회색 채움 카드(테두리 최소) / 토스 블루 강조 / 큰 라운드·굵은 타이포
#   - 한글·숫자 모두 Pretendard. 색상은 :root 변수만 바꾸면 전체 톤 변경 가능.
#   * 로직과 무관한 순수 스타일 레이어.
# ==========================================
st.markdown("""
<style>
...
:root{
  --bg:#ffffff; --fill:#f2f4f6; --fill-2:#f9fafb;
  --txt-0:#191f28; --txt-1:#4e5968; --txt-2:#8b95a1;
  --accent:#3182f6; --accent-soft:#e8f1ff; --accent-deep:#1b64da;
  --sans:'Pretendard','Pretendard Variable',-apple-system,BlinkMacSystemFont,sans-serif;
}

/* ===== 앱 배경 ===== */
.stApp{background:var(--bg); color:var(--txt-0); font-family:var(--sans);}
...
h1,h2,h3,h4{font-family:var(--sans); color:var(--txt-0); letter-spacing:-0.03em; font-weight:800;}
h1{font-size:2rem;} h2{font-size:1.7rem; margin-top:.3rem;}

/* ===== 사이드바 ===== */
section[data-testid="stSidebar"]{background:var(--fill-2); border-right:1px solid var(--fill);}
...
section[data-testid="stSidebar"] div[role="radiogroup"]>label{
  display:flex; align-items:center; width:100%; padding:11px 14px; border-radius:14px; margin:0;
  color:var(--txt-1); font-size:.93rem; font-weight:600; cursor:pointer; border:none; transition:all .14s;}
section[data-testid="stSidebar"] div[role="radiogroup"]>label:hover{background:var(--fill); color:var(--txt-0);}
section[data-testid="stSidebar"] div[role="radiogroup"]>label>div:first-child{display:none;}
section[data-testid="stSidebar"] div[role="radiogroup"]>label:has(input:checked){
  background:var(--accent-soft); color:var(--accent-deep); font-weight:800;}

/* ===== 지표(Metric) = 연회색 채움 카드 ===== */
[data-testid="stMetric"]{background:var(--fill); border:none; border-radius:22px; padding:18px 20px;}
[data-testid="stMetricLabel"]{color:var(--txt-2)!important; font-size:.82rem!important; font-weight:700;}
[data-testid="stMetricValue"]{color:var(--txt-0)!important; font-weight:800; letter-spacing:-0.03em;}

/* ===== 버튼 ===== */
.stButton>button,.stDownloadButton>button,.stFormSubmitButton>button{
  font-family:var(--sans); font-weight:700; letter-spacing:-0.01em;
  background:var(--fill); color:var(--txt-0); border:none; border-radius:14px; padding:.6rem 1.1rem; transition:all .14s;}
.stButton>button:hover,.stFormSubmitButton>button:hover{background:#e7ebf0; color:var(--txt-0);}
.stButton>button:active{transform:scale(.985);}
.stButton>button[kind="primary"],.stFormSubmitButton>button[kind="primaryFormSubmit"]{
  background:var(--accent); color:#fff; font-weight:800; box-shadow:0 6px 16px rgba(49,130,246,.28);}
.stButton>button[kind="primary"]:hover{background:var(--accent-deep); color:#fff;}

/* ===== 본문 가로 라디오 = 알약 토글 ===== */
.main div[role="radiogroup"]{gap:9px; flex-wrap:wrap;}
.main div[role="radiogroup"]>label{
  background:var(--fill); border:none; border-radius:999px; padding:7px 16px; color:var(--txt-1); font-weight:700; transition:all .14s;}
.main div[role="radiogroup"]>label:hover{background:#e7ebf0; color:var(--txt-0);}
.main div[role="radiogroup"]>label:has(input:checked){background:var(--accent); color:#fff;}
.main div[role="radiogroup"]>label>div:first-child{display:none;}

/* ===== 입력/셀렉트 ===== */
.stTextInput input,.stNumberInput input,.stTextArea textarea,
[data-baseweb="select"]>div,[data-baseweb="input"]{
  background:var(--fill)!important; border:1px solid transparent!important; border-radius:14px!important; color:var(--txt-0)!important;}
.stTextInput input:focus,.stNumberInput input:focus,[data-baseweb="select"]>div:focus-within{
  background:#fff!important; border-color:var(--accent)!important; box-shadow:0 0 0 3px var(--accent-soft)!important;}

/* ===== 탭 ===== */
.stTabs [data-baseweb="tab-list"]{gap:4px; border-bottom:1px solid var(--fill);}
.stTabs [data-baseweb="tab"]{background:transparent; color:var(--txt-1); font-weight:700; border-radius:10px 10px 0 0; padding:9px 16px;}
.stTabs [aria-selected="true"]{color:var(--accent); border-bottom:2.5px solid var(--accent);}

/* ===== 익스팬더 ===== */
[data-testid="stExpander"]{border:none!important; border-radius:18px!important; background:var(--fill); overflow:hidden;}
[data-testid="stExpander"] summary{padding:14px 18px; color:var(--txt-0); font-weight:700;}
[data-testid="stExpander"] summary:hover{color:var(--accent);}

/* ===== 데이터프레임 / 테이블 ===== */
.stDataFrame,[data-testid="stTable"]{border:1px solid var(--fill); border-radius:18px; overflow:hidden;}
table{border-collapse:collapse; width:100%;}
thead th{background:var(--fill-2)!important; color:var(--txt-2)!important; font-size:.8rem; font-weight:700!important;
  border-bottom:1px solid var(--fill)!important;}
tbody td{color:var(--txt-0); border-bottom:1px solid var(--fill)!important; font-weight:600;}
tbody tr:hover{background:var(--fill-2);}

/* ===== 알림 (네이티브 시맨틱 색 유지, 모서리만 둥글게) ===== */
div[data-testid="stAlert"]{border-radius:18px;}

/* ===== 진행바 ===== */
.stProgress>div>div>div{background:var(--accent)!important;}
.stProgress>div>div{background:var(--fill)!important;}

/* ===== 구분선 ===== */
hr,[data-testid="stDivider"]{border:none!important; height:1px!important; background:var(--fill)!important;}

/* ===== 차트 컨테이너 ===== */
[data-testid="stPlotlyChart"],[data-testid="stVegaLiteChart"]{
  background:var(--fill-2); border:1px solid var(--fill); border-radius:20px; padding:10px;}

/* ===== 스크롤바 ===== */
::-webkit-scrollbar{width:10px; height:10px;}
::-webkit-scrollbar-track{background:var(--bg);}
::-webkit-scrollbar-thumb{background:#dfe3ea; border-radius:8px; border:2px solid var(--bg);}
::-webkit-scrollbar-thumb:hover{background:#cfd5df;}
