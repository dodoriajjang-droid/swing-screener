...
st.markdown("""
<style>
    div[role="tablist"] { flex-wrap: wrap !important; gap: 6px !important; padding-bottom: 10px !important; }
    button[role="tab"] { flex: 1 1 12% !important; min-width: 130px !important; background-color: #f1f3f6 !important; border: 1px solid #d1d5db !important; border-radius: 8px !important; padding: 8px 5px !important; margin: 0 !important; display: flex !important; justify-content: center !important; }
    button[role="tab"][aria-selected="true"] { background-color: #ff4b4b !important; color: white !important; border-color: #ff4b4b !important; font-weight: bold !important; }
    button[role="tab"][aria-selected="true"] p { color: white !important; }
</style>
...
# ==========================================
# 2. 통합 데이터 수집 & AI 함수 모음 (최상단 고정 - NameError 방지)
# ==========================================
...
# 👈 [핵심 업데이트 2] 네이버 IPO 무적의 수동 파싱 로직 적용
@st.cache_data(ttl=10800)
def get_naver_ipo_data():
    try:
        url = "https://finance.naver.com/sise/ipo.naver"
        res = requests.get(url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/122.0.0.0 Safari/537.36'})
        soup = BeautifulSoup(res.content.decode('euc-kr', 'replace'), 'html.parser')
        ...
        # 화면에 출력할 핵심 열만 솎아내기 (희망공모가 등 변동되는 열 방어)
        valid_cols = []
...
# ETF 및 배당주 포트폴리오 데이터를 최대한 압축하여 코드 라인수 절약
    ...
    with st.expander(expander_title, expanded=is_expanded):
        ...
        with st.spinner(f"{tf} 차트 데이터 불러오는 중..."):
            ...
            if not long_df.empty:
                ...
                else: st.caption("해외 주식이거나 세부 수급 데이터를 제공하지 않는 종목입니다.")
...
def display_sorted_results(results_list, tab_key, api_key=""):
    ...
    elif "PER 낮은순" in sort_opt: sorted_res = sorted(display_list, key=lambda x: get_safe_float(x['PER'], 9999))
    elif "PBR 낮은순" in sort_opt: sorted_res = sorted(display_list, key=lambda x: get_safe_float(x['PBR'], 9999))
...
# 4. 메인 화면 시작 (UI 구성)
# ==========================================
st.title("📈 Jaemini PRO 트레이딩 터미널")
st.markdown("단기 스윙 매매를 위한 **실시간 수급 추적** 및 **타점/펀더멘털 통합 모니터링** 시스템입니다.")
...
with tab0:
    ...
    def draw_gauge(val, prev, title, steps, is_error=False):
        if is_error: return go.Figure(go.Indicator(mode="gauge", value=50, title={'text': f"<b>{title}</b><br><span style='font-size:12px;color:red'>서버 통신 차단됨</span>"}, gauge={'axis': {'range': [0, steps[-1]['range'][1]]}, 'bar': {'color': "gray"}}))
                ...
                for idx, row in kings_df.head(10).iterrows():
                    ...
                    if res:
                        if res['RSI'] <= 35:
                            ...
                            signal_count += 1
                if signal_count == 0:
                    st.caption("현재 상위 10개 주도주 중 특별한 기술적(매수) 시그널이 포착된 종목이 없습니다.")
            else:
                st.caption("정규장 오픈 전이거나 실시간 데이터를 수집할 수 없습니다.")
    ...
    with q_col2:
        ...
        if not st.session_state.watchlist:
            ...
            for item in st.session_state.watchlist:
                ...
                if res:
                    if res['현재가'] <= res['손절가']:
                        st.error(f"🔴 **손절가 이탈 위험:** {item['종목명']} (현재: {res['현재가']:,}원 / 손절선: {res['손절가']:,}원)")
                    elif res['현재가'] >= res['목표가1'] * 0.98:
                        st.success(f"🟢 **익절 구간 도달:** {item['종목명']} (현재: {res['현재가']:,}원 / 1차목표: {res['목표가1']:,}원)")
                    else:
                        st.warning(f"🟡 **홀딩 대기중:** {item['종목명']} (현재: {res['현재가']:,}원)")
