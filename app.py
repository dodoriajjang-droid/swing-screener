...
# 👈 [핵심 업데이트] 라디오 버튼을 탭(버튼)처럼 보이게 만드는 마법의 CSS
st.markdown("""
<style>
    div[role="radiogroup"] {
        display: flex;
...
# 👈 [핵심 업데이트] 1~2페이지 초고속 수집 + 2.5초 타임아웃 강제 컷오프
...
def get_latest_naver_news():
    ...
    def fetch_page(page):
        try:
            ...
            res = requests.get(url, headers={"User-Agent": "Mozilla/5.0"}, timeout=2.5) # 타임아웃 짧게 (무한로딩 방지)
    ...
    # 두 개의 페이지를 동시에(스레드) 긁어와서 속도 2배 향상
...
with st.sidebar:
    ...
    if st.button("🔄 현재 화면 새로고침", type="primary", use_container_width=True): 
        st.rerun()
    st.divider()
...
# 👈 [핵심 업데이트] 속도를 높이기 위해 st.tabs를 버리고 st.radio 메뉴로 대체 (게으른 로딩)
menu_list = [
...
if selected_menu == "🎛️ 메인 대시보드":
    ...
    with col2:
        ...
        if sel_tick != "N/A" and api_key_input:
            ...
            with st.spinner('✨ AI 추천 국내 수혜주 탐색 중...'):
                kor_stocks = get_ai_matched_stocks(sel_tick, sec, ind, sel_opt.split(" - ")[0], api_key_input)
                if kor_stocks:
                    theme_res_list = [res for name, code in kor_stocks if (res := analyze_technical_pattern(name, code))]
